...
export default function PipelineCard({ application, onStageChange, onClick }: PipelineCardProps) {
  ...
  return (
    <div
      className="bg-white rounded-xl border border-claude-border p-3 cursor-pointer hover:shadow-sm transition-shadow shadow-sm"
      onClick={onClick}
    >
      <div className="flex items-center gap-1.5">
        <span className="text-xs text-claude-tertiary font-medium">{job?.company ?? "Unknown"}</span>
      ...
      <div className="text-sm font-medium text-claude-primary mt-0.5 line-clamp-2">
        {job?.title ?? "Untitled"}
      </div>
      {job && (
        <div className={`text-xs mt-1 ${job.salary_min || job.salary_max ? "text-emerald-700 font-medium" : "text-claude-tertiary"}`}>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {application.next_action && (
          <span className="text-xs text-claude-accent truncate max-w-[120px]">
            {application.next_action}
          </span>
        )}
      </div>
      <select
        ...
        className="mt-2 w-full text-xs border border-claude-border rounded-lg px-2 py-1 bg-white"
      >
